...
func TestFuncAIR(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{}, &tengo.Int{Value: 10})
	assert.NoError(t, err)
	assert.Equal(t, tengo.UndefinedValue, ret)
	_, err = funcCall(uf, mockInterop{})
	assert.Equal(t, tengo.ErrWrongNumArguments, err)
}

func TestFuncAR(t *testing.T) {
	uf := stdlib.FuncAR(func() {})
	ret, err := funcCall(uf, mockInterop{})
	assert.NoError(t, err)
	assert.Equal(t, tengo.UndefinedValue, ret)
	_, err = funcCall(uf, mockInterop{}, tengo.TrueValue)
	assert.Equal(t, tengo.ErrWrongNumArguments, err)
}
...
func TestFuncARI(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{})
	assert.NoError(t, err)
	assert.Equal(t, &tengo.Int{Value: 10}, ret)
	_, err = funcCall(uf, mockInterop{}, tengo.TrueValue)
	assert.Equal(t, tengo.ErrWrongNumArguments, err)
}
...
func TestFuncARE(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{})
	assert.NoError(t, err)
	assert.Equal(t, tengo.TrueValue, ret)
	uf = stdlib.FuncARE(func() error { return errors.New("some error") })
	ret, err = funcCall(uf, mockInterop{})
	assert.NoError(t, err)
	assert.Equal(t, &tengo.Error{Value: &tengo.String{Value: "some error"}}, ret)
	_, err = funcCall(uf, mockInterop{}, tengo.TrueValue)
	assert.Equal(t, tengo.ErrWrongNumArguments, err)
}
...
func TestFuncARIsE(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{})
	assert.NoError(t, err)
	assert.Equal(t, array(&tengo.Int{Value: 1}, &tengo.Int{Value: 2}, &tengo.Int{Value: 3}), ret)
...
func TestFuncARS(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{})
	assert.NoError(t, err)
	assert.Equal(t, &tengo.String{Value: "foo"}, ret)
	_, err = funcCall(uf, mockInterop{}, tengo.TrueValue)
	assert.Equal(t, tengo.ErrWrongNumArguments, err)
}
...
func TestFuncARSs(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{})
	assert.NoError(t, err)
	assert.Equal(t, array(&tengo.String{Value: "foo"}, &tengo.String{Value: "bar"}), ret)
	_, err = funcCall(uf, mockInterop{}, tengo.TrueValue)
	assert.Equal(t, tengo.ErrWrongNumArguments, err)
}
...
func TestFuncASRE(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{}, &tengo.String{Value: "foo"})
	assert.NoError(t, err)
	assert.Equal(t, tengo.TrueValue, ret)
...
func TestFuncASRSs(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{}, &tengo.String{Value: "foo"})
	assert.NoError(t, err)
	assert.Equal(t, array(&tengo.String{Value: "foo"}), ret)
	_, err = funcCall(uf, mockInterop{})
	assert.Equal(t, tengo.ErrWrongNumArguments, err)
}
...
func TestFuncASI64RE(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{}, &tengo.String{Value: "foo"}, &tengo.Int{Value: 5})
	assert.NoError(t, err)
	assert.Equal(t, tengo.TrueValue, ret)
...
func TestFuncAIIRE(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{}, &tengo.Int{Value: 5}, &tengo.Int{Value: 7})
	assert.NoError(t, err)
	assert.Equal(t, tengo.TrueValue, ret)
...
func TestFuncASIIRE(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{}, &tengo.String{Value: "foo"}, &tengo.Int{Value: 5}, &tengo.Int{Value: 7})
	assert.NoError(t, err)
	assert.Equal(t, tengo.TrueValue, ret)
...
func TestFuncASSRE(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{}, &tengo.String{Value: "foo"}, &tengo.String{Value: "bar"})
	assert.NoError(t, err)
	assert.Equal(t, tengo.TrueValue, ret)
	...
	assert.NoError(t, err)
	assert.Equal(t, &tengo.Error{Value: &tengo.String{Value: "some error"}}, ret)
	_, err = funcCall(uf, mockInterop{}, &tengo.String{Value: "foo"})
	assert.Equal(t, tengo.ErrWrongNumArguments, err)
}

func TestFuncASsRS(t *testing.T) {
	uf := stdlib.FuncASsSRS(func(a []string, b string) string { return strings.Join(a, b) })
	ret, err := funcCall(uf, mockInterop{}, array(&tengo.String{Value: "foo"}, &tengo.String{Value: "bar"}), &tengo.String{Value: " "})
	assert.NoError(t, err)
	assert.Equal(t, &tengo.String{Value: "foo bar"}, ret)
	_, err = funcCall(uf, mockInterop{}, &tengo.String{Value: "foo"})
	assert.Equal(t, tengo.ErrWrongNumArguments, err)
}
...
func TestFuncARF(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{})
	assert.NoError(t, err)
	assert.Equal(t, &tengo.Float{Value: 10.0}, ret)
	_, err = funcCall(uf, mockInterop{}, tengo.TrueValue)
	assert.Equal(t, tengo.ErrWrongNumArguments, err)
}
...
func TestFuncAFRF(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{}, &tengo.Float{Value: 10.0})
	assert.NoError(t, err)
	assert.Equal(t, &tengo.Float{Value: 10.0}, ret)
	_, err = funcCall(uf, mockInterop{})
	assert.Equal(t, tengo.ErrWrongNumArguments, err)
	_, err = funcCall(uf, mockInterop{}, tengo.TrueValue, tengo.TrueValue)
	assert.Equal(t, tengo.ErrWrongNumArguments, err)
}
...
func TestFuncAIRF(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{}, &tengo.Int{Value: 10.0})
	assert.NoError(t, err)
	assert.Equal(t, &tengo.Float{Value: 10.0}, ret)
	_, err = funcCall(uf, mockInterop{})
	assert.Equal(t, tengo.ErrWrongNumArguments, err)
...
func TestFuncAFRI(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{}, &tengo.Float{Value: 10.5})
	assert.NoError(t, err)
	assert.Equal(t, &tengo.Int{Value: 10}, ret)
	_, err = funcCall(uf, mockInterop{})
	assert.Equal(t, tengo.ErrWrongNumArguments, err)
...
func TestFuncAFRB(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{}, &tengo.Float{Value: 0.1})
	assert.NoError(t, err)
	assert.Equal(t, tengo.TrueValue, ret)
	_, err = funcCall(uf, mockInterop{})
	assert.Equal(t, tengo.ErrWrongNumArguments, err)
...
func TestFuncAFFRF(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{}, &tengo.Float{Value: 10.0}, &tengo.Float{Value: 20.0})
	assert.NoError(t, err)
	assert.Equal(t, &tengo.Float{Value: 30.0}, ret)
	_, err = funcCall(uf, mockInterop{})
	assert.Equal(t, tengo.ErrWrongNumArguments, err)
	_, err = funcCall(uf, mockInterop{}, tengo.TrueValue)
	assert.Equal(t, tengo.ErrWrongNumArguments, err)
...
func TestFuncASIRS(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{}, &tengo.String{Value: "ab"}, &tengo.Int{Value: 2})
	assert.NoError(t, err)
	assert.Equal(t, &tengo.String{Value: "abab"}, ret)
	_, err = funcCall(uf, mockInterop{})
	assert.Equal(t, tengo.ErrWrongNumArguments, err)
	_, err = funcCall(uf, mockInterop{}, tengo.TrueValue)
	assert.Equal(t, tengo.ErrWrongNumArguments, err)
...
func TestFuncAIFRF(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{}, &tengo.Int{Value: 10}, &tengo.Float{Value: 20.0})
	assert.NoError(t, err)
	assert.Equal(t, &tengo.Float{Value: 30.0}, ret)
	_, err = funcCall(uf, mockInterop{})
	assert.Equal(t, tengo.ErrWrongNumArguments, err)
...
func TestFuncAFIRF(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{}, &tengo.Float{Value: 10.0}, &tengo.Int{Value: 20})
	assert.NoError(t, err)
	assert.Equal(t, &tengo.Float{Value: 30.0}, ret)
	_, err = funcCall(uf, mockInterop{})
	assert.Equal(t, tengo.ErrWrongNumArguments, err)
...
func TestFuncASSIRSs(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{}, &tengo.String{Value: "foo"}, &tengo.String{Value: "bar"}, &tengo.Int{Value: 5})
	assert.NoError(t, err)
	assert.Equal(t, array(&tengo.String{Value: "foo"}, &tengo.String{Value: "bar"}, &tengo.String{Value: "5"}), ret)
	_, err = funcCall(uf, mockInterop{})
	assert.Equal(t, tengo.ErrWrongNumArguments, err)
}
...
func TestFuncARYE(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{})
	assert.NoError(t, err)
	assert.Equal(t, &tengo.Bytes{Value: []byte("foo bar")}, ret)
	uf = stdlib.FuncARYE(func() ([]byte, error) {
		return nil, errors.New("some error")
	})
	ret, err = funcCall(uf, mockInterop{})
...
func TestFuncASRIE(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{}, &tengo.String{Value: "foo"})
	assert.NoError(t, err)
	assert.Equal(t, &tengo.Int{Value: 5}, ret)
...
func TestFuncAYRIE(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{}, &tengo.Bytes{Value: []byte("foo")})
	assert.NoError(t, err)
	assert.Equal(t, &tengo.Int{Value: 5}, ret)
...
func TestFuncASSRI(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{}, &tengo.String{Value: "foo"}, &tengo.String{Value: "bar"})
	assert.NoError(t, err)
	assert.Equal(t, &tengo.Int{Value: 6}, ret)
	_, err = funcCall(uf, mockInterop{}, &tengo.String{Value: "foo"})
	assert.Equal(t, tengo.ErrWrongNumArguments, err)
}
...
func TestFuncASSRS(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{}, &tengo.String{Value: "foo"}, &tengo.String{Value: "bar"})
	assert.NoError(t, err)
	assert.Equal(t, &tengo.String{Value: "foobar"}, ret)
	_, err = funcCall(uf, mockInterop{}, &tengo.String{Value: "foo"})
	assert.Equal(t, tengo.ErrWrongNumArguments, err)
}
...
func TestFuncASSRB(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{}, &tengo.String{Value: "123"}, &tengo.String{Value: "12"})
	assert.NoError(t, err)
	assert.Equal(t, tengo.TrueValue, ret)
	_, err = funcCall(uf, mockInterop{}, &tengo.String{Value: "foo"})
	assert.Equal(t, tengo.ErrWrongNumArguments, err)
...
func TestFuncAIRS(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{}, &tengo.Int{Value: 55})
	assert.NoError(t, err)
	assert.Equal(t, &tengo.String{Value: "55"}, ret)
	_, err = funcCall(uf, mockInterop{})
	assert.Equal(t, tengo.ErrWrongNumArguments, err)
}
...
func TestFuncAIRIs(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{}, &tengo.Int{Value: 55})
	assert.NoError(t, err)
	assert.Equal(t, array(&tengo.Int{Value: 55}, &tengo.Int{Value: 55}), ret)
	_, err = funcCall(uf, mockInterop{})
	assert.Equal(t, tengo.ErrWrongNumArguments, err)
}
...
func TestFuncARI64(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{})
	assert.NoError(t, err)
	assert.Equal(t, &tengo.Int{Value: 55}, ret)
	_, err = funcCall(uf, mockInterop{}, &tengo.Int{Value: 55})
	assert.Equal(t, tengo.ErrWrongNumArguments, err)
}

func TestFuncASsSRS(t *testing.T) {
	uf := stdlib.FuncASsSRS(func(a []string, b string) string { return strings.Join(a, b) })
	ret, err := funcCall(uf, mockInterop{}, array(&tengo.String{Value: "abc"}, &tengo.String{Value: "def"}), &tengo.String{Value: "-"})
	assert.NoError(t, err)
	assert.Equal(t, &tengo.String{Value: "abc-def"}, ret)
	_, err = funcCall(uf, mockInterop{})
	assert.Equal(t, tengo.ErrWrongNumArguments, err)
}
...
func TestFuncAI64RI64(t *testing.T) {
	...
	ret, err := funcCall(uf, mockInterop{}, &tengo.Int{Value: 55})
	assert.NoError(t, err)
	assert.Equal(t, &tengo.Int{Value: 110}, ret)
	_, err = funcCall(uf, mockInterop{})
	assert.Equal(t, tengo.ErrWrongNumArguments, err)
}
