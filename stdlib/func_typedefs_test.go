...
func TestFuncAIR(t *testing.T) {
	...
	ret, err := funcCall(uf, mockRuntime{}, &objects.Int{Value: 10})
	assert.NoError(t, err)
	assert.Equal(t, objects.UndefinedValue, ret)
	_, err = funcCall(uf, mockRuntime{})
	assert.Equal(t, objects.ErrWrongNumArguments, err)
}

func TestFuncAR(t *testing.T) {
	uf := stdlib.FuncAR(func() {})
	ret, err := funcCall(uf, mockRuntime{})
	assert.NoError(t, err)
	assert.Equal(t, objects.UndefinedValue, ret)
	_, err = funcCall(uf, mockRuntime{}, objects.TrueValue)
	assert.Equal(t, objects.ErrWrongNumArguments, err)
}
...
func TestFuncASRE(t *testing.T) {
	...
	ret, err := funcCall(uf, mockRuntime{}, &objects.String{Value: "foo"})
	assert.NoError(t, err)
	assert.Equal(t, objects.TrueValue, ret)
...
func TestFuncASI64RE(t *testing.T) {
	...
	ret, err := funcCall(uf, mockRuntime{}, &objects.String{Value: "foo"}, &objects.Int{Value: 5})
	assert.NoError(t, err)
	assert.Equal(t, objects.TrueValue, ret)
...
func TestFuncAIIRE(t *testing.T) {
	...
	ret, err := funcCall(uf, mockRuntime{}, &objects.Int{Value: 5}, &objects.Int{Value: 7})
	assert.NoError(t, err)
	assert.Equal(t, objects.TrueValue, ret)
...
func TestFuncASIIRE(t *testing.T) {
	...
	ret, err := funcCall(uf, mockRuntime{}, &objects.String{Value: "foo"}, &objects.Int{Value: 5}, &objects.Int{Value: 7})
	assert.NoError(t, err)
	assert.Equal(t, objects.TrueValue, ret)
...
func TestFuncASSRE(t *testing.T) {
	...
	ret, err := funcCall(uf, mockRuntime{}, &objects.String{Value: "foo"}, &objects.String{Value: "bar"})
	assert.NoError(t, err)
	assert.Equal(t, objects.TrueValue, ret)
	...
	assert.NoError(t, err)
	assert.Equal(t, &objects.Error{Value: &objects.String{Value: "some error"}}, ret)
	_, err = funcCall(uf, mockRuntime{}, &objects.String{Value: "foo"})
	assert.Equal(t, objects.ErrWrongNumArguments, err)
}

func TestFuncASsRS(t *testing.T) {
	uf := stdlib.FuncASsSRS(func(a []string, b string) string { return strings.Join(a, b) })
	ret, err := funcCall(uf, mockRuntime{}, array(&objects.String{Value: "foo"}, &objects.String{Value: "bar"}), &objects.String{Value: " "})
	assert.NoError(t, err)
...
func TestFuncAFRF(t *testing.T) {
	...
	ret, err := funcCall(uf, mockRuntime{}, &objects.Float{Value: 10.0})
	assert.NoError(t, err)
	assert.Equal(t, &objects.Float{Value: 10.0}, ret)
	_, err = funcCall(uf, mockRuntime{})
	assert.Equal(t, objects.ErrWrongNumArguments, err)
	_, err = funcCall(uf, mockRuntime{}, objects.TrueValue, objects.TrueValue)
	assert.Equal(t, objects.ErrWrongNumArguments, err)
}
...
func TestFuncAIRF(t *testing.T) {
	...
	ret, err := funcCall(uf, mockRuntime{}, &objects.Int{Value: 10.0})
	assert.NoError(t, err)
	assert.Equal(t, &objects.Float{Value: 10.0}, ret)
	_, err = funcCall(uf, mockRuntime{})
	assert.Equal(t, objects.ErrWrongNumArguments, err)
...
func TestFuncAFRI(t *testing.T) {
	...
	ret, err := funcCall(uf, mockRuntime{}, &objects.Float{Value: 10.5})
	assert.NoError(t, err)
	assert.Equal(t, &objects.Int{Value: 10}, ret)
	_, err = funcCall(uf, mockRuntime{})
	assert.Equal(t, objects.ErrWrongNumArguments, err)
...
func TestFuncAFRB(t *testing.T) {
	...
	ret, err := funcCall(uf, mockRuntime{}, &objects.Float{Value: 0.1})
	assert.NoError(t, err)
	assert.Equal(t, objects.TrueValue, ret)
	_, err = funcCall(uf, mockRuntime{})
	assert.Equal(t, objects.ErrWrongNumArguments, err)
...
func TestFuncAFFRF(t *testing.T) {
	...
	ret, err := funcCall(uf, mockRuntime{}, &objects.Float{Value: 10.0}, &objects.Float{Value: 20.0})
	assert.NoError(t, err)
...
func TestFuncASIRS(t *testing.T) {
	...
	ret, err := funcCall(uf, mockRuntime{}, &objects.String{Value: "ab"}, &objects.Int{Value: 2})
	assert.NoError(t, err)
...
func TestFuncAIFRF(t *testing.T) {
	...
	ret, err := funcCall(uf, mockRuntime{}, &objects.Int{Value: 10}, &objects.Float{Value: 20.0})
	assert.NoError(t, err)
	assert.Equal(t, &objects.Float{Value: 30.0}, ret)
	_, err = funcCall(uf, mockRuntime{})
	assert.Equal(t, objects.ErrWrongNumArguments, err)
...
func TestFuncAFIRF(t *testing.T) {
	...
	ret, err := funcCall(uf, mockRuntime{}, &objects.Float{Value: 10.0}, &objects.Int{Value: 20})
	assert.NoError(t, err)
	assert.Equal(t, &objects.Float{Value: 30.0}, ret)
	_, err = funcCall(uf, mockRuntime{})
	assert.Equal(t, objects.ErrWrongNumArguments, err)
...
func TestFuncASSIRSs(t *testing.T) {
	...
	ret, err := funcCall(uf, mockRuntime{}, &objects.String{Value: "foo"}, &objects.String{Value: "bar"}, &objects.Int{Value: 5})
	assert.NoError(t, err)
...
func TestFuncAYRIE(t *testing.T) {
	...
	ret, err := funcCall(uf, mockRuntime{}, &objects.Bytes{Value: []byte("foo")})
	assert.NoError(t, err)
	assert.Equal(t, &objects.Int{Value: 5}, ret)
...
func TestFuncASSRB(t *testing.T) {
	...
	ret, err := funcCall(uf, mockRuntime{}, &objects.String{Value: "123"}, &objects.String{Value: "12"})
	assert.NoError(t, err)
	assert.Equal(t, objects.TrueValue, ret)
	_, err = funcCall(uf, mockRuntime{}, &objects.String{Value: "foo"})
	assert.Equal(t, objects.ErrWrongNumArguments, err)
...
func TestFuncAIRS(t *testing.T) {
	...
	ret, err := funcCall(uf, mockRuntime{}, &objects.Int{Value: 55})
	assert.NoError(t, err)
...
func TestFuncARI64(t *testing.T) {
	...
	_, err = funcCall(uf, mockRuntime{}, &objects.Int{Value: 55})
	assert.Equal(t, objects.ErrWrongNumArguments, err)
}

func TestFuncASsSRS(t *testing.T) {
	uf := stdlib.FuncASsSRS(func(a []string, b string) string { return strings.Join(a, b) })
	ret, err := funcCall(uf, mockRuntime{}, array(&objects.String{Value: "abc"}, &objects.String{Value: "def"}), &objects.String{Value: "-"})
	assert.NoError(t, err)
...
func funcCall(fn objects.CallableFunc, rt objects.Runtime, args ...objects.Object) (objects.Object, error) {
