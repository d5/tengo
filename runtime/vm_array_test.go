...
func TestArray(t *testing.T) {
	...
	// array index set
	expectErrorString(t, `a1 := [1, 2, 3]; a1[3] = 5`, "index out of bounds")
	...
	expectErrorString(t, fmt.Sprintf("%s[:%d]", arrStr, -1), "invalid slice index")
	expectErrorString(t, fmt.Sprintf("%s[%d:]", arrStr, arrLen+1), "invalid slice index")
	expectErrorString(t, fmt.Sprintf("%s[%d:%d]", arrStr, 0, -1), "invalid slice index")
	expectErrorString(t, fmt.Sprintf("%s[%d:%d]", arrStr, 2, 1), "invalid slice index")
}
